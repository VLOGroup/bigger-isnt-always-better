...
@utils.register_model(name='msncsn')
class MSNCSN(nn.Module):
  """MSNCSN model"""
  ...
  def forward(self, x, time_cond, d):
    ...
    if self.progressive_input != 'none':
      input_pyramid = input_modules[d - 1](x)
    ...
    for i_level in range(4-d, self.num_resolutions):
      # Residual blocks for this resolution
    ...
    # Upsampling block
    for i_level in reversed(range(4-d, self.num_resolutions)):
