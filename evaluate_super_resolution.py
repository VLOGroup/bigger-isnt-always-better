...
def get_P(factor, kernel):
    l = len(kernel)
    kernel = np.array(kernel, dtype=np.float32)
    kernel = np.outer(kernel, kernel)
    kernel /= np.sum(kernel)
    kernel = torch.from_numpy(kernel).view(1, 1, l, l).cuda()

    def P(x):
        #return torch.repeat_interleave(torch.repeat_interleave(x[:, :, ::factor, ::factor], repeats=factor, dim=2), repeats=factor, dim=3)
        return nn.functional.conv2d(torch.repeat_interleave(torch.repeat_interleave(nn.functional.conv2d(x, kernel, padding='same')[:, :, ::factor, ::factor], repeats=factor, dim=2), repeats=factor, dim=3), kernel, padding='same')
...
def main():
    ...
    ssim = SSIM().cuda()
    test_dl = mydata.create_celeba_dataloader(1000)
    ...
    pc_fouriercs = get_pc_supersampling(sde,
                                        predictor, corrector,
                                        inverse_scaler,
                                        snr=snr,
                                        n_steps=m,
    ...
    for i, img in enumerate(test_dl):
        ...
        P = get_P(8, [1, 3, 3, 1])
        img_under = torch.repeat_interleave(torch.repeat_interleave(img[:, :, ::8, ::8], repeats=8, dim=2), repeats=8, dim=3)
        plt.imsave('iu.png', img_under.squeeze().detach().cpu().numpy())
        
        x = pc_fouriercs(score_model, img_under, P)
        print(torch.max(x))

        plt.imsave('i.png', img.squeeze().detach().cpu().numpy())
        plt.imsave('d.png', x.squeeze().detach().cpu().numpy())
        break
