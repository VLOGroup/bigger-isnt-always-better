...
root = Path('/media/lukasglaszner/data/results_ready')
masks = {'mri': ['g1d4', 'g1d8', 'g2d4', 'radial', 'poisson'], 'ct': ['sv60', 'sv30', 'sv20', 'fanbeam_60']}
datasets = {'mri': ['corpd', 'corpdfs', 'brain'], 'ct': ['thorax', 'head']}
algorithms = ['ald', 'pc', 'dps']
datatypes = {'mri': ['fastmri', 'celeba'], 'ct': ['ct', 'celeba']}
models = ['4_attention', '4', '3', '2', '1']
            ...
            for k, dataset in enumerate(datasets[methodology]):
                ...
                for j, model in enumerate(models):
                    for i, mask in enumerate(masks[methodology]):
                        ...
                        try:
                            psnr = np.loadtxt((root / f'{datatype}_{model}_{dataset}_{mask}_{algorithm}{"_new" if algorithm == "pc" and mask == "fanbeam_60" else ""}' / 'psnr_values').with_suffix('.csv'), delimiter=',')
                            ssim = np.loadtxt((root / f'{datatype}_{model}_{dataset}_{mask}_{algorithm}{"_new" if algorithm == "pc" and mask == "fanbeam_60" else ""}' / 'ssim_values').with_suffix('.csv'), delimiter=',')
