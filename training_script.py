...
import numpy as np
import logging
from models import mrncsn, msncsn, ncsnpp
import losses
import sampling
...
def train(model_config, data_config, training_config, sample_config, workdir, device, logger=None):
  ...
  if (workdir / 'loss.npy').is_file():
    loss_over_iter = np.load(workdir / 'loss.npy')
  logger.info(f'Dataset size = {num_data}')
  ...
  train_step_fn = losses.get_step_fn(sde, train=True, optimize_fn=optimize_fn,
                                     ...
                                     likelihood_weighting=likelihood_weighting, depth_param=False)
  ...
  if training_config['training']['snapshot_sampling']:
    ...
    sampling_fn = sampling.get_sampling_fn(sample_config, sde, sampling_shape, inverse_scaler, continuous, sampling_eps, device, depth_param=False)
  ...
  if (workdir / 'training_times_epoch.csv').is_file():
    training_times_epoch = np.loadtxt(workdir / 'training_times_epoch.csv', dtype=float, delimiter=',')
  if (workdir / 'training_times_batch.csv').is_file():
    training_times_batch = np.loadtxt(workdir / 'training_times_batch.csv', dtype=float, delimiter=',')

  print(f'{sum(p.numel() for p in score_model.parameters())} total parameters in the model.')
  ...
  for epoch in range(initial_batch, training_config['training']['epochs']):
    ...
    for step, batch in enumerate(train_dl):
      try:
        ...
        loss_over_iter[epoch * num_data + step] = loss.detach().cpu().numpy()
    ...
    if training_config['training']['snapshot_sampling'] and (epoch % training_config['training']['sample_frequency'] == 0 or epoch == training_config['training']['epochs'] - 1):
      ...
      try:
        with torch.no_grad():
          sample, _ = sampling_fn(score_model)
          sample_to_save = sample.squeeze().detach().cpu().numpy()
          np.save(this_sample_dir / f'sample', sample_to_save)

          iio.imwrite(this_sample_dir /  f'sample.png', np.clip(sample_to_save * 255, 0, 255).astype(np.uint8))
          del sample_to_save
          del sample
      except Exception as e:
      ...
      logger.info(f'CUDA  Memory Allocated: {torch.cuda.memory_allocated()}')
  if not 'path' in model_config:
    model_config['path'] = checkpoint_dir / 'checkpoint.pth'
  return
